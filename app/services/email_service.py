...
# # =========================================================
# # Attachments (shared contract for SMTP + Mailgun)
# # =========================================================
# class EmailAttachment(TypedDict, total=False):
#     filename: str                 # required
#     content_type: str             # required (e.g. application/pdf)
#     data: bytes                   # required (raw bytes)
#     disposition: str              # optional (default "attachment")
#     content_id: str               # optional (for inline images etc)
...
#                     auth=("api", self.cfg.mailgun_api_key),
#                     data=data,
#                     files=files,          # ✅ attachments go here
#                     timeout=20,
#                 )
#                 Log.info(f"Mailgun send status={resp.status_code} attempt={attempt}")
...
#         # ✅ Attachments (Mailgun uses multipart form with files=)
#         files: Optional[List[tuple]] = None
#         if attachments:
#             files = []
#             for a in attachments:
...
#                 if not filename or not blob:
#                     continue
#                 # ("attachment", (filename, filebytes, mimetype))
#                 files.append(("attachment", (filename, blob, content_type)))
...
#         return {"ok": True, "provider": "mailgun", "status_code": resp.status_code, "response": payload}
...
#     def send(self, to, subject, text, html=None, reply_to=None, tags=None, meta=None, attachments=None):
#         to_list = [to] if isinstance(to, str) else to
...
#         # ✅ Attachments (Gmail needs proper MIME parts)
#         if attachments:
#             for a in attachments:
#                 filename = a.get("filename") or "attachment"
#                 content_type = a.get("content_type") or "application/octet-stream"
...
# # ---------------------------------
# # EMAIL TO USER UPON PAYMENT SUCCESS (with invoice attachment)
# #----------------------------------
# def send_payment_confirmation_email(
#     email: str,
#     fullname: str,
...
#     package_amount: Optional[float] = None,
#     total_from_amount: Optional[float] = None,
#     addon_users: Optional[float] = None,
#     invoice_pdf_path: Optional[str] = None,
#     invoice_pdf_bytes: Optional[bytes] = None,
#     invoice_filename: Optional[str] = None,
# ) -> Dict[str, Any]:
#     """
#     Sends payment confirmation email and attaches invoice PDF.
#     Provide either invoice_pdf_path or invoice_pdf_bytes.
#     """
...
#     if invoice_pdf_bytes:
#         attachments.append({
#             "filename": invoice_filename or f"invoice-{invoice_number}.pdf",
#             "content_type": "application/pdf",
#             "data": invoice_pdf_bytes,
#         })
#     elif invoice_pdf_path:
#         with open(invoice_pdf_path, "rb") as f:
#             pdf_bytes = f.read()
#         attachments.append({
#             "filename": invoice_filename or os.path.basename(invoice_pdf_path) or f"invoice-{invoice_number}.pdf",
#             "content_type": "application/pdf",
#             "data": pdf_bytes,
#         })
